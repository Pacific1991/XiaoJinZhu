...
#import "HCStraRatingView.h"

@implementation HCStraRatingView
...
- (void)baseInit {
    _notSelectedStar = [UIImage imageNamed:@"good_star_gray"];
    _selectedStar = [UIImage imageNamed:@"good_star_red"];
    _halfSelectedStar = [UIImage imageNamed:@"good_star_half"];
...
- (void)handleTouchAtLocation:(CGPoint)touchLocation {
    ...
    _rating = 0;
    for(NSInteger i = _starViews.count - 1; i >= 0; i--) {
        UIImageView *imageView = [_starViews objectAtIndex:i];
    ...
    [self refreshStars];
    if ([self.DElegate respondsToSelector:@selector(changeLabvelWithRating:)]) {
        [self.DElegate changeLabvelWithRating:_rating];
    }
}
