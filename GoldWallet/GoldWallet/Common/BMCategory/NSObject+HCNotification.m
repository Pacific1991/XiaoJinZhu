...
@interface _HCBlockTarget : NSObject
/**添加一个KVOBlock*/
- (void)hc_addBlock:(void(^)(__weak id obj, id oldValue, id newValue))block;
- (void)hc_addNotificationBlock:(void(^)(NSNotification *notification))block;

- (void)hc_doNotification:(NSNotification*)notification;
@end

@implementation _HCBlockTarget{
    //保存所有的block
...
@implementation NSObject (HCNotification)
static void *const HCKVOBlockKey = "HCKVOBlockKey";
static void *const HCNotificationBlockKey = "HCNotificationBlockKey";
static void * deallocHasSwizzledKey = "deallocHasSwizzledKey";

- (void)hc_addObserverBlockForKeyPath:(NSString*)keyPath block:(void (^)(id obj, id oldVal, id newVal))block {
    if (!keyPath || !block) return;
    //取出存有所有KVOTarget的字典
    NSMutableDictionary *allTargets = objc_getAssociatedObject(self, HCKVOBlockKey);
    if (!allTargets) {
        //没有则创建
        allTargets = [NSMutableDictionary new];
        //绑定在该对象中
        objc_setAssociatedObject(self, HCKVOBlockKey, allTargets, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    //获取对应keyPath中的所有target
    _HCBlockTarget *targetForKeyPath = allTargets[keyPath];
    if (!targetForKeyPath) {
        //没有则创建
        targetForKeyPath = [_HCBlockTarget new];
        //保存
    ...
    [targetForKeyPath hc_addBlock:block];
    //对第一次注册KVO的类进行dealloc方法调剂
    [self _hc_swizzleDealloc];
}

- (void)hc_removeObserverBlockForKeyPath:(NSString *)keyPath{
    if (!keyPath.length) return;
    NSMutableDictionary *allTargets = objc_getAssociatedObject(self, HCKVOBlockKey);
    if (!allTargets) return;
    _HCBlockTarget *target = allTargets[keyPath];
    if (!target) return;
    [self removeObserver:target forKeyPath:keyPath];
    [allTargets removeObjectForKey:keyPath];
}

- (void)hc_removeAllObserverBlocks {
    NSMutableDictionary *allTargets = objc_getAssociatedObject(self, HCKVOBlockKey);
    if (!allTargets) return;
    [allTargets enumerateKeysAndObjectsUsingBlock:^(id key, _HCBlockTarget *target, BOOL *stop) {
...
- (void)hc_addNotificationForName:(NSString *)name block:(void (^)(NSNotification *notification))block {
    if (!name || !block) return;
    NSMutableDictionary *allTargets = objc_getAssociatedObject(self, HCNotificationBlockKey);
    if (!allTargets) {
        allTargets = @{}.mutableCopy;
        objc_setAssociatedObject(self, HCNotificationBlockKey, allTargets, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    _HCBlockTarget *target = allTargets[name];
    if (!target) {
        target = [_HCBlockTarget new];
        allTargets[name] = target;
        [[NSNotificationCenter defaultCenter] addObserver:target selector:@selector(hc_doNotification:) name:name object:nil];
    }
    [target hc_addNotificationBlock:block];
    [self _hc_swizzleDealloc];
}

- (void)hc_removeNotificationForName:(NSString *)name{
    if (!name) return;
    NSMutableDictionary *allTargets = objc_getAssociatedObject(self, HCNotificationBlockKey);
...
- (void)hc_removeAllNotification{
    NSMutableDictionary *allTargets = objc_getAssociatedObject(self, HCNotificationBlockKey);
    if (!allTargets.count) return;
    [allTargets enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, _HCBlockTarget *target, BOOL * _Nonnull stop) {
        [[NSNotificationCenter defaultCenter] removeObserver:target];
    }];
    [allTargets removeAllObjects];
}

- (void)hc_postNotificationWithName:(NSString *)name userInfo:(NSDictionary *)userInfo{
...
- (void)_hc_swizzleDealloc{
    ...
    id newDealloc = ^(__unsafe_unretained id objSelf){
        //在这里我们移除所有的KVO
        [objSelf hc_removeAllObserverBlocks];
        //移除所有通知
        [objSelf hc_removeAllNotification];
