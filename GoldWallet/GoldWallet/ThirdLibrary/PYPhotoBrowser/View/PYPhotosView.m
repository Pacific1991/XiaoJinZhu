...
- (void)longTapPhotoViewsSubView:(UILongPressGestureRecognizer *)longTap {
    
    if (longTap.state == UIGestureRecognizerStateBegan) {
        PYPhotoView *view = (PYPhotoView *)longTap.view;
        if (self.delegate && [self.delegate respondsToSelector:@selector(photosView:didLongTapPhotoView:)]) {
            [self.delegate photosView:self didLongTapPhotoView:view];
        }
    }
}
...
- (void)setPhotos:(NSArray<PYPhoto *> *)photos
{
    ...
    while (self.subviews.count < photoCount) { // UIImageView不够，需要创建
        ...
        UILongPressGestureRecognizer *longTap = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(longTapPhotoViewsSubView:)];
